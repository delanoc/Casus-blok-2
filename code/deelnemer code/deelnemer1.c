...
const char* antwoordenArray[][4] = {
	...
	{"A: Aanraking", "B: Baksteen", "C: Insecten", "D: Spoken"},
};
...
void loop()
{
	...
	// Welcome message
	lcd.clear(); lcd.setCursor(0, 0); lcd.print("Ronde begint..");
	lcd.setCursor(0, 1); lcd.print("Ronde: "); lcd.print(rondeNummer);
	delay(2000);
	
	if (questionIndex < 20) {gamemodeID = 1;} // meerkeuze vraag
	else if (questionIndex > 19) {gamemodeID = 0;} // buzzer vraag
...
void beantwoordMeerkeuze()
{
		...
		while (!selected) {
			...
			while (!pressed) {
				...
				lcd.clear(); lcd.print("antwoord:"); lcd.setCursor(0, 1); lcd.print(antwoordenArray[questionIndex][printIndex]);
				delay(200);
			}
		}
		if (printIndex == 1) {antwoord = "A";}
		else if (printIndex == 2) {antwoord = "B";}
		else if (printIndex == 3) {antwoord = "C";}
		else if (printIndex == 4) {antwoord = "D";}
}


void beantwoordBuzzer()
{
	lcd.clear(); lcd.print("druk om te"); lcd.setCursor(0, 1); lcd.print("antwoorden");
	...
	reactieijtdString.toCharArray(reactietijd,4);
}
